...
AVCapturePhotoCaptureDelegate, StreamerEngineDelegate, CompositeImageLayerDelegate {
    ...
    weak var imageLayerPreview: ImagePreviewOverlay? {
        didSet {
            if let image = imageLayer.outputImage, let layer = imageLayerPreview {
    ...
    var pauseMode: PauseMode = .off {
        didSet {
            ...
            switch pauseMode {
            case .off:
                silenceGenerator.setOverlays(overlayIds: [])
            case .pause:
                silenceGenerator.setOverlays(overlayIds: Settings.sharedInstance.pauseLayers)
            case .standby:
                silenceGenerator.setOverlays(overlayIds: Settings.sharedInstance.standbyLayers)
    ...
    weak var uvMeter: AudioLevelMeter?
    ...
    func recordStateDidChange(_ state: RecordState, url: URL?) {
        switch state {
        case .initialized:
            isRecording = true
        case .started:
        ...
        workQueue.async {
            do {
                ...
                if startVideo {
                    ...
                    self.imageLayer.loadActiveOnly()
                }
    ...
    internal func setCameraParams(camera: AVCaptureDevice) -> AVCaptureDevice.Format? {
        ...
        if initZoom > format.videoMaxZoomFactor {
            DDLogWarn("Zoom factor \(initZoom) is out of range")
            initZoom = format.videoMaxZoomFactor
            Settings.sharedInstance.backCameraZoom = initZoom
        }
    ...
    @objc private func sessionWasInterrupted(notification: Notification) {
        if let userInfoValue = notification.userInfo?[AVCaptureSessionInterruptionReasonKey] as AnyObject?, let reasonIntegerValue = userInfoValue.integerValue, let reason = AVCaptureSession.InterruptionReason(rawValue: reasonIntegerValue) {
            ...
            if reason == .videoDeviceNotAvailableInBackground {
                return // Session will be stopped by Presuno app when it goes to background, ignore notification
            }
    ...
    internal func processsAudioSampleBuffer(_ sampleBuffer: CMSampleBuffer, fromOutput audioDataOutput: AVCaptureAudioDataOutput) {
        uvMeter?.putBuffer(sampleBuffer)
    ...
    // Presuno sets the following metadata properties and values.
