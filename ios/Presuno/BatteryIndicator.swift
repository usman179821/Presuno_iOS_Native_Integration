...
class BatteryIndicator: CALayer {
    var displayThreshold: CGFloat = 100 // In percents
    var yellowThreshold: CGFloat = 35
    var redThreshold: CGFloat = 25
    ...
    @objc func onBatteryChange(_ notification: Notification) {
        ...
        DDLogInfo(String(format:"Batterey change %3.2f%%", device.batteryLevel * 100))
        batteryCharge = CGFloat(device.batteryLevel)
        batteryState = device.batteryState
        setNeedsLayout()
    }
