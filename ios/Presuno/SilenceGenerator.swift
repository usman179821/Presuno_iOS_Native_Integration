...
class SilenceGenerator: CompositeImageLayerDelegate {
    ...
    func setStreamSize(width: Int, height: Int) {
        ...
        if !overlayIds.isEmpty {
            pauseOverlay.loadIdList(overlayIds)
            displayOverlay = true
            overlayIds.removeAll()
        }
    }
    
    func setOverlays(overlayIds: [Int64]) {
        if streamWidth > 0 && streamHeight > 0 {
            pauseOverlay.loadIdList(overlayIds)
            displayOverlay = !overlayIds.isEmpty
    ...
    func outputBlackFrame(withPresentationTime time: CMTime) {
        ...
        if (blackFrame == nil) {
            ...
            if displayOverlay, let image = pauseOverlay.outputImage {
                ...
                ciContent.render(image, to: blackFrame!)
                imageLayerPreview?.setPauseImage(pauseOverlay.outputImage?.cgImage)
    ...
    func onImageLoadComplete() {
        if displayOverlay,
           let image = pauseOverlay.outputImage,
           let blackFrame = blackFrame {
            let context = CIContext()
            context.render(image, to: blackFrame)
            imageLayerPreview?.setPauseImage(pauseOverlay.outputImage?.cgImage)
        }
