...
fileprivate extension String {
    func toArrayOfNumberic() -> [Int64] {
        let strArr = self.split(separator: ",")
        let intArr: [Int64] = strArr.map { Int64($0) ?? 0 }
        return intArr
    }
}
...
class Settings: SettingsKeys {
    ...
    var showLayersPreview: Bool {
        get {
            return UserDefaults.standard.bool(forKey: SK.image_layer_preview_key)
        } set {
            UserDefaults.standard.setValue(newValue, forKey: SK.image_layer_preview_key)
        }
    }

    var streamStartInStandby: Bool {
        get {
            return UserDefaults.standard.bool(forKey: SK.image_layer_standby_key)
        } set {
            UserDefaults.standard.setValue(newValue, forKey: SK.image_layer_standby_key)
        }
    }

    var standbyLayers: [Int64] {
        get {
            let value = UserDefaults.standard.string(forKey: SK.image_layer_standby_images_key) ?? ""
            return value.toArrayOfNumberic()
        } set {
            let strArr: [String] = newValue.map { String($0) }
            let str = strArr.joined(separator: ",")
            UserDefaults.standard.setValue(str, forKey: SK.image_layer_standby_images_key)
        }
    }

    var pauseLayers: [Int64] {
        get {
            let value = UserDefaults.standard.string(forKey: SK.image_layer_pause_images_key) ?? ""
            return value.toArrayOfNumberic()
        } set {
            let strArr: [String] = newValue.map { String($0) }
            let str = strArr.joined(separator: ",")
            UserDefaults.standard.setValue(str, forKey: SK.image_layer_pause_images_key)
        }
    }
    ...
    var videoConfig: VideoConfig {
        ...
        Toast(text: message, theme: .info, layout: .statusLine)
    ...
    var postprocess: Bool {
        let value = UserDefaults.standard.bool(forKey: SK.core_image_key)
        if value == false {
            return value
        } else {
    ...
    func getDefaultBackCamera(probe:((AVCaptureDevice, CMVideoDimensions, Double) -> Bool)? = nil) -> AVCaptureDevice? {
        ...
        if typeStr != camera_type_default {
            if typeStr != nil {
                let message = NSLocalizedString("Selected back camera is missing, switching to Auto", comment: "")
